...
>```
>filtra y muestra únicamente los commits de fusión en una sola línea, mientras que wc -l cuenta el total de renglones recibidos para obtener el número exacto.
...
>git branch -a
>```
lista todas las ramas (tanto locales como remotas). Al filtrar con grep -v, restamos los punteros de referencia administrativos para quedarnos únicamente con la cantidad real de ramas de trabajo creadas.

## Análisis de Impacto y Conflictos Históricos
### 5. Commit con Mayor Cantidad de Archivos Modificados
Hash del Commit: **` 40e9795 `**

Cantidad de Archivos Involucrados: 4

Comando empleado para auditar el impacto:

```bash
git show 40e9795 --stat
```
>[!NOTE]
